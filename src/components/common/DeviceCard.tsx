...
export const DeviceCard: React.FC<DeviceCardProps> = ({
  ...
  const handleStartMirror = (e: React.MouseEvent) => {
    e.stopPropagation();
    startSession({ serial: device.serial });
  };

  const handleMirrorScreenOff = (e: React.MouseEvent) => {
    e.stopPropagation();
    startSession({ serial: device.serial, turnScreenOff: true });
  };
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-5 text-xs font-mono">
        <div className="p-2 rounded-lg bg-surface border border-border">
          <span className="text-[10px] text-text-muted uppercase block">Serial</span>
        ...
        <div className="p-2 rounded-lg bg-surface border border-border">
          <span className="text-[10px] text-text-muted uppercase block">OS Version</span>
          <span className="text-text-primary font-semibold block">
            {device.androidVersion ? `Android ${device.androidVersion}` : 'Unknown'}
          </span>
        </div>

        <div className="p-2 rounded-lg bg-surface border border-border">
          <span className="text-[10px] text-text-muted uppercase block">API Level</span>
          <span className="text-text-primary font-semibold block">
        ...
        <div className="p-2 rounded-lg bg-surface border border-border">
          <span className="text-[10px] text-text-muted uppercase block">Display</span>
          <span className="text-text-primary font-semibold block">
