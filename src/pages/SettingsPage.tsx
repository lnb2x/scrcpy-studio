...
export const SettingsPage: React.FC = () => {
  ...
  const handleRestartAdb = async () => {
    setAdbServerStatus('Restarting ADB server...');
    try {
      await invoke('adb_kill_server');
      await invoke('adb_start_server');
      setAdbServerStatus('ADB Server restarted successfully.');
    } catch (e) {
      setAdbServerStatus(`Failed to restart ADB server: ${String(e)}`);
    }
  };

  const handleKillAdb = async () => {
    setAdbServerStatus('Killing ADB server...');
    try {
      await invoke('adb_kill_server');
      setAdbServerStatus('ADB Server terminated.');
    } catch (e) {
      setAdbServerStatus(`Failed to kill ADB server: ${String(e)}`);
    }
  };
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-fade-in pb-16">
      ...
      <div>
        ...
        <p className="text-xs text-text-secondary mt-1">
          Configure executable locations, appearance themes, language, and storage defaults.
        </p>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-text-primary">Executables & Toolchains</h3>
            <p className="text-xs text-text-secondary mt-0.5">
              Paths to scrcpy and Android Debug Bridge binaries
            </p>
          </div>
          ...
          >
            ...
            <span>{isDetecting ? 'Scanning...' : t('autoDetect')}</span>
          </button>
        </div>
            ...
            <label className="text-xs font-semibold text-text-primary flex items-center gap-1.5">
              ...
              {detection?.isScrcpyReady ? (
                <span className="text-[10px] text-emerald-400 font-mono flex items-center gap-0.5">
                  <CheckCircle2 className="w-3 h-3" /> Ready ({detection.scrcpyVersion || '4.1'})
                </span>
              ) : (
          ...
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-text-primary flex items-center gap-1.5">
              ...
              {detection?.isAdbReady ? (
                <span className="text-[10px] text-emerald-400 font-mono flex items-center gap-0.5">
                  <CheckCircle2 className="w-3 h-3" /> Ready ({detection.adbVersion || '1.0.41'})
                </span>
              ) : (
                <span className="text-[10px] text-rose-400 font-mono flex items-center gap-0.5">
                  <XCircle className="w-3 h-3" /> Not Found
        ...
        <div className="pt-3 border-t border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="text-xs text-text-muted">
            {adbServerStatus || 'ADB Server daemon management'}
          </div>
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary">Visual styles and color palette</p>
          </div>
        </div>
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary">Application display language</p>
          </div>
        </div>
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-text-primary">Storage Directories</h3>
            <p className="text-xs text-text-secondary">Default save locations on local machine</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              />
              <button
                onClick={() => handleBrowseFolder('recordingsDir')}
                className="px-3.5 py-2 rounded-lg bg-surface-hover hover:bg-surface-active text-text-secondary text-xs font-medium border border-border"
              >
                Browse...
              </button>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-text-primary">Screenshots Folder</label>
            <div className="flex gap-2">
              <input
                type="text"
                readOnly
              ...
              />
              <button
                onClick={() => handleBrowseFolder('screenshotsDir')}
                className="px-3.5 py-2 rounded-lg bg-surface-hover hover:bg-surface-active text-text-secondary text-xs font-medium border border-border"
              >
                Browse...
              </button>
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary">v0.1.0 (Target: scrcpy 4.1)</p>
            </div>
          </div>
        ...
        <p className="text-xs text-text-muted leading-relaxed pt-2 border-t border-border">
          {t('poweredBy')}. Scrcpy is developed by Romain Vimont and Genymobile contributors under
